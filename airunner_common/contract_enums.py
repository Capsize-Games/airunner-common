"""Canonical cross-process and cross-layer contract enums.

This is the single source of truth for the enums that must agree between the
desktop GUI (``src/``) and the headless daemon (``services/``). The GUI's
historical ``airunner.enums`` module keeps its own GUI-only enums and remains
unchanged; the shared ``SignalCode`` and model/lifecycle enums live here.

Deliberate drift-resolution decision (see architecture audit O1):

* ``SignalCode.TTS_GENERATOR_WORKER_ADD_TO_STREAM_SIGNAL`` canonicalized to
  ``"TTSGeneratorWorker_add_to_stream_signal"``. The services copy had drifted
  to lowercase ``"tts_generator_worker_add_to_stream"``. The mixed-case value
  is the intended value: it matches both the value actually emitted by
  ``airunner_services.workers.tts_generator_worker`` and the listener-method
  naming convention used by the GUI vocalizer worker.
"""
